...
import com.catsimulator.model.ModelData;
...
public class ModelLoader 
{
	...
	public Model loadToVAO(float[] points, float[] textureCoords, int[] indices)
	{
		int vaoId = createVAO();
	...
	public int loadTexture(String textureFilePath)
	{
		Texture texture = null;
		
		try 
		{
			texture = TextureLoader.getTexture("PNG", new FileInputStream("resources/" + textureFilePath + ".png"));
	...
	{
		ModelData data = OBJLoaderV2.loadOBJ(fileName);
		Model model = loadToVAO(data.getVertices(), data.getTextureCoords(), data.getIndices());
		return model;
	}
